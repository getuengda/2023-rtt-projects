package perscholas_junit_testing;
...
public class PasswordComply {
	...
	public boolean doesNotAlreayExist() throws SQLException {
		Connection conn = DriverManager.getConnection(DB_URL, USER, passwordString);
		CallableStatement stmt = conn.prepareCall(QUERY);

		stmt.setString(1, passwordString);
		stmt.registerOutParameter(1, java.sql.Types.VARCHAR);
		System.out.println("Executing strored procedure ...");
		stmt.execute();
	...
	public boolean doesPasswordComply() {
		...
		return verifyPasswordLength() && verifyAplhaNumeric() && hasAllowedspecailCharacters()
				&& hasNoSpecailCharacters();
	}
